    ...
    def compute_sample_model_loss(self, hidden_state,
        ...
        for b in range(hidden_state.size(0)):
            if pos_len[b] > 0 and (model_embedding.size(1) - pos_len[b]) > 0:
                ...
                # weights = torch.arange(pos_len[b], 0, -1, dtype=torch.float32, device=simi.device)  # (topk,)
                # weights = weights / weights.sum()  # 归一化权重
                # weights = weights.unsqueeze(0).expand(topk_v.size(0), -1)

                # 加权求和 topk 部分
                # weighted_topk = (topk_v * weights).sum()
                # weighted_topk = topk_v.sum(dim = -1)#/topk
                # lastk_v = torch.clamp(lastk_v, max=torch.exp(-self.margin)).sum()
                lastk_v = lastk_v.sum()
                for l in range(pos_len[b]):
                    loss_ += -torch.log(topk_v[l]/(topk_v[l] + lastk_v + 1e-8))
                loss.append(loss_)
        #         Top_k.append(weighted_topk)
        #         Last_v.append(lastk_v)
        # Top_k = torch.stack(Top_k)
        # Last_v = torch.stack(Last_v)
        # lastk_v = lastk_v
...
def save_model(path, model, config, step, save_model = True):
...
def train(config):
    ...
    local_save_path = os.path.join(config.save_folder, formatted_time)
    ...
    for epoch in tqdm(range(config.epochs)):
        ...
        for batch1 in routerloader:
            ...
            sample_model_loss_ = routermodel.compute_sample_model_loss(hidden_state1, model_embedding,model_index, topk = 7, lastk = 7, pos_len = pos_len)
            ...
            if (step1 + 1) % config.log_steps == 0:
                print(f"##Epoch {epoch} Stage 1 ## Step {step1 + 1}, projector-lr: {optimizer.param_groups[0]['lr']}, other-lr: {optimizer.param_groups[1]['lr']}, sample-model loss: {sample_model_loss_.item()}, model-perf loss: {model_perf_loss_.item()}, sample-cluster loss: {sample_cluster_loss_.item()}")#, cluster-cluster loss: {cluster_cluster_loss_.item()}") #, sample-cluster loss: {sample_cluster_loss_.item()}") #, cluster-cluster loss: {cluster_cluster_loss_.item()}")

                    
            if (step1 + 1) % config.eval_steps == 0:
                score_ = evaluation(routermodel, config, tokenizer, device, local_save_path, step1 + 1, True,  False)
                if step1 + 1 >= 200:
                    pass
                    _ = evaluation(routermodel, config, tokenizer, device, local_save_path, step1 + 1, True,  True)
    
            # if (step1 + 1) % config.save_steps == 0:
            #     if score_ > score:
            #         save_model(local_save_path, routermodel, config, step1 + 1)
            #         score = score_
